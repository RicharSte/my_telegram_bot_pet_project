...
def anketa_name(update, context):
    ...
    if len(user_name.split()) < 2:
        ...
        update.message.reply_text(
            'Пожалуеста оцените нашего бота от 1 до 5',
...
def anketa_rating(update, context):
    ...
    update.message.reply_text('Напишите коментарий или нажмите /skip чтобы пропустить')
    return 'comment'
...
def anketa_comment(update, context):
    ...
    update.message.reply_text(user_text, reply_markup=key_board(), parse_mode=ParseMode.HTML)
